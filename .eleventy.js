...
module.exports = function (config) {
  ...
  config.addFilter("isTocEmpty", (content) => {
    return (
      ...
      content === "" ||
      content === " " ||
      content === '<nav class="toc" ><ol></ol></nav>'
    );
  });
  ...
  config.addCollection("sideNavigation", function (collectionApi) {
    ...
    const mainFolders = fs
      ...
      .filter((dirent) => {
        ...
        return (
          dirent.isDirectory() && fileExists(path.join(folderPath, ".SIDENAV"))
        );
      });
            ...
            const children = contentFiles
              .map((file) => {
                ...
                if (frontMatter?.showInSidenav) {
                  ...
                  const page = collectionApi
                    .getAll()
                    .find((entry) => entry.inputPath.endsWith(filePath));
  ...
  let markdownLibrary = markdownIt({
    ...
    .use(function (md) {
      ...
      md.renderer.rules.link_open = function (tokens, idx, options, env, self) {
        ...
        const hrefAttr = token.attrs
          ? token.attrs.find((attr) => attr[0] === "href")
          : null;
